...
insert into Questions(Title, Description, CategoryId, CreatedByUserId, DateCreated, DateLastModified) values('Gas pours out of the breather when engine starts up', 'My 2002 jaguar x type v6 3.0 starts up and gas pours out of the breather. Please help. Changed gaskets on the intake top and bottom but it still just pours out', 1, 1028, getdate() - 26, getdate() - 26)
...
insert into Questions(Title, Description, CategoryId, CreatedByUserId, DateCreated, DateLastModified) values('Blower motor location', 'Where would I be able to locate the blower motor on a jetta?',1, 1032, getdate() - 18, getdate() - 18)
...
insert into Questions(Title, Description, CategoryId, CreatedByUserId, DateCreated, DateLastModified) values('How to prevent rust', 'I have recently notice that my vehicle has started to get rust spots and this has become concerning for me, I would like to know what recomendations people have for dealing with rust', 6, 1031, getdate(), getdate())
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Engine data
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(1, 'The exciter wire gets its power through the amp warning light. Key on, alt stopped, the bulb lights, once the alternator is turning it makes power and the amp light goes off', 1018, 0, getdate() - 28, getdate() - 28)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(3, 'It seems like a huge vacuum leak. It could be the intake manifold or base carburetor gasket', 1025, 0, getdate() - 26, getdate() - 26)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(4, 'Chances are, your battery is at fault when your car will not start casuse cold temperatures can cause a chemical reaction in your vehicles battery to exponentially slow down, a weak battery can often cause your car not to start.', 1009, 0, getdate() - 24, getdate() - 24)


--Transmission
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(18, 'You would need to know what exact transmission your ranger has to get to the right filter kit, consider doing some research on the exact model you have', 1002, 0, getdate() - 10,  getdate() - 10)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(23, 'Possibility of low transmission fluid as low fluid could prevent your car from engaging the drive', 1005, 0, getdate() - 4, getdate() - 4)


-- General Maintenance
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(25, 'One of the reasons why dash lights stay on after key is removed includes a short in the control module for the lights.' , 1008, 0, getdate() - 30, getdate() - 30)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(25, 'If the lights are on even when the key to the ignition is taken out, the majority of times it is an issue with the ignition switch.', 1010, 0, getdate() - 29, getdate() - 29)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(26, 'Best bet would be to replace the fan speed control panel. It should come off as one section for the knobs but can sometimes be a pain to get to to remove it and you basically have to take out the entire head unit to do it. Make sure to note if your ac controls are manual or automatic. That is a very important difference.', 1012, 0, getdate() - 25, getdate() - 25)


--Auto Electrical
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(37, 'The nagtive side of a cars battery connects to the vehicles chassis', 1015, 0, getdate() -20, getdate() -20)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(38, 'The red colored terminal is for positive and the black colored terminal is for negative', 1017, 0, getdate() - 10, getdate() - 10)
insert into Answers(QuestionId, Description, AnsweredByUserId, IsAcceptedAnswer, DateCreated, DateLastModified) values(39,'Most of the electrical power required by the electrical and electronic devices in a vehicle is provided by the alternator', 1020, 0, getdate() -5, getdate() -5)